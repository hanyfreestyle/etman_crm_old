...
if($row['user_id'] == $RowUsreInfo['user_id'] or $AdminConfig['leads'] == '1' ){
    ...
    if($row['visit_s'] == '0' and $row['c_type'] != '5' and $row['state'] == '2' ){
        ...
        $Err[] = NF_PrintInput("Date",$AdminLangFile['ticket_visit_date'],"visit_date","1","0","req",$MoreS);

        echo '<div style="clear: both!important;"></div>';

        $MoreS = array('Col' => "col-md-12",'Placeholder'=> "",'required' => 'required');
        $Err[] = NF_PrintInput("TextArea",$AdminLangFile['ticket_visit_des'],"des","1","0","req",$MoreS);
        ...
        if(isset($_POST['B1'])){
            if($Err != '1'){
                Vall($Err,"AddVisit",$db,"1",$USER_PERMATION_Edit);
            }
        }
