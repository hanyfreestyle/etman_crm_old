...
if($row['user_id'] == $RowUsreInfo['user_id'] or $AdminConfig['leads'] == '1' ){
    ...
    if($row['rev_s'] == '1' and $row['cancel_s'] == '0' and $row['state'] == '2'){
...
//hidden
        ...
        $Err[] = NF_PrintInput("Date2",$AdminLangFile['ticket_cancel_date'],"cancel_date","1","0","req",$MoreS);

        echo '<div style="clear: both!important;"></div>';

        $MoreS = array('Col' => "col-md-12",'Placeholder'=> "",'required' => 'required');
        $Err[] = NF_PrintInput("TextArea",$AdminLangFile['ticket_cancel_des'],"des","1","0","req",$MoreS);
        ...
        if(isset($_POST['B1'])){
            if($Err != '1'){
                Vall($Err,"CancelReservation",$db,"1",$USER_PERMATION_Edit);
            }
        }
